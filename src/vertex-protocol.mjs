...
export function supportsNativeRequest(payload) {
  ...
  if (format?.type === "json_schema" && (!object(format.json_schema) || format.json_schema.strict === true)) return false;
...
function candidateMessage(candidate) {
  ...
  for (const part of candidate.content?.parts ?? []) {
    ...
    if (part.functionCall) {
      ...
      if (fc.partialArgs || typeof fc.name !== "string" || !object(fc.args)) throw new Error("invalid_native_tool");
...
export function translateNativeCompletion(native, model) {
  if (!object(native) || native.error) throw new Error("invalid_native_completion");
  let candidates = native.candidates;
  if (!candidates?.length && native.promptFeedback?.blockReason) candidates = [{ finishReason: "SAFETY" }];
  if (!Array.isArray(candidates) || !candidates.length) throw new Error("invalid_native_completion");
  return { id: "chatcmpl-" + randomUUID(), object: "chat.completion", created: Math.floor(Date.now() / 1000), model,
    choices: candidates.map((c, i) => {
      ...
      if (!c.finishReason) throw new Error("incomplete_native_completion");
...
export function wrapNativeStream(response, model, onUsage = () => {}) {
  ...
  const event = (controller, raw) => {
    ...
    if (parsed.error) throw new Error("native_stream_error");
    ...
    for (const candidate of candidates) {
      ...
      if (state.done) throw new Error("native_data_after_finish");
  ...
  const drain = controller => {
    for (;;) {
      const match = /\r\n\r\n|\n\n/.exec(buffer);
      if (!match) break;
      if (match.index > 2 * 1024 * 1024) throw new Error("native_event_limit");
      const raw = buffer.slice(0, match.index); buffer = buffer.slice(match.index + match[0].length); event(controller, raw);
    }
    if (buffer.length > 2 * 1024 * 1024) throw new Error("native_event_limit");
  };
  const body = response.body.pipeThrough(new TransformStream({
    ...
    flush(c) {
      ...
      if (!states.size || [...states.values()].some(s => !s.done)) throw new Error("native_stream_interrupted");
